    ...
    def step(self, obs):
        ...
        if len(self.actions) == 0:
            ...
            if idle_workers > 0:
                ...
                print(PROTOSS_MACROS.Train_Probe, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Train_Probe]))
                ...
                print(PROTOSS_MACROS.Train_Stalker, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Train_Stalker]))
                ...
                print(PROTOSS_MACROS.Collect_Gas, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Collect_Gas]))
                self.actions = PROTOSS_MACROS.Collect_Gas()
            elif self.can_build():
                if food < 4 and mineral > 100:
                    print(PROTOSS_MACROS.Build_Pylon, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Build_Pylon]))
                    ...
                    print(PROTOSS_MACROS.Build_Assimilator, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Build_Assimilator]))
                    ...
                    print(PROTOSS_MACROS.Build_CyberneticsCore, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Build_CyberneticsCore]))
                    self.actions = PROTOSS_MACROS.Build_CyberneticsCore()
                    self.last_build_frame = self.frame
                elif mineral > 150 and self.get_unit_counts(obs, units.Protoss.Gateway) < 4:
                    print(PROTOSS_MACROS.Build_Gateway, adapter.transform([obs], [last_obs], [PROTOSS_MACROS.Build_Gateway]))
