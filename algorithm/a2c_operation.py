...
class A2COperation(BaseDeepAgent, BaseSC2Agent):
    ...
    def step(self, state=None, **kwargs):
        operation_step = kwargs['step'] % 8 == 0
        zealot_num = kwargs['obs']._unit_counts.get(units.Protoss.Zealot, 0)
        if operation_step or zealot_num < 4:
    ...
    def update(self, states, actions,
               next_states, rewards, dones, *args, **kwargs):
        operation_step = kwargs['step'] % 8 == 0
        zealot_num = kwargs['obs']._unit_counts.get(units.Protoss.Zealot, 0)
        if not operation_step and zealot_num >= 4:
            return
        super().update()
